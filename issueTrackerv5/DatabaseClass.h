...
//=================================================================================================================================================
	...
	bool ConfirmLogin(String^ username, String^ pass)
	{
		...
		sqlCmd->CommandText = "SELECT firstName, lastName, Username, Password FROM mydb.UserInfo WHERE Username = @username AND Password = @password;";
...
//=================================================================================================================================================
	void updateIssueList(int issueid, String^ title, String^ descrip, int assignto, int issuetypeid, int issuepriorityid, int issuestatusid)
	{
		sqlCmd->Connection = sqlConn;
		try
		{
			//sqlCmd->CommandText = "UPDATE mydb.IssueList SET Title = '" + title + "', Description = '" + descrip + "', UserID = '" + assignto + "', TypeList_TypeID = '" + issuetypeid + "', PriorityID = '" + issuepriorityid + "', StatusID = '" + issuestatusid + "' WHERE IssueID = '" + issueid + "';";
			sqlCmd->CommandText = "UPDATE `mydb`.`IssueList` SET `Title` = '" + title + "', `Description` = '" + descrip + "', `UserID` = '" + assignto + "', `TypeList_TypeID` = '" + issuetypeid + "', `PriorityID` = '" + issuepriorityid + "', `StatusID` = '" + issuestatusid + "' WHERE (`IssueID` = '" + issueid + "');";
...
//=================================================================================================================================================
	void deleteIssueList(int issueid)
	{
		sqlCmd->Connection = sqlConn;
		try
		{
			sqlCmd->CommandText = "DELETE FROM `mydb`.`IssueList` WHERE(`IssueID` = '" + issueid + "');";
